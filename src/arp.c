...
void arp_in(buf_t *buf)
{
    ...
    else
    {
        ...
        if(opcode == ARP_REQUEST && memcmp(arp_pkt->target_ip, net_if_ip, NET_IP_LEN) == 0 )
        {
            ...
            //初始化ARP报头
            send_head->hw_type = swap16(ARP_HW_ETHER),
            send_head->pro_type = swap16(NET_PROTOCOL_IP),
            send_head->hw_len = NET_MAC_LEN,
            send_head->pro_len = NET_IP_LEN,
            send_head->opcode = swap16(ARP_REPLY);
            //发送方ip和mac
            memcpy(send_head->sender_ip,net_if_ip,NET_IP_LEN);
            memcpy(send_head->sender_mac,net_if_mac,NET_MAC_LEN);
            //接收方ip和mac
            memcpy(send_head->target_ip, arp_pkt->sender_ip, NET_IP_LEN);
            memcpy(send_head->target_mac, arp_pkt->sender_mac, NET_MAC_LEN);
            //发送ARP reply报文
